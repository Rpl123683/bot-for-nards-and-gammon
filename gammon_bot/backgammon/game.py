...
class Game:
    ...
    def empty_home_slots(self,player):
        count=0
        home_part=self.grid[18:24]
        for i in home_part:
            if player not in i:
    ...
    def Race(self,player):
        ...
        for i in range(len(self.grid))[::-1]:
            if self.opponent(player) in self.grid[i]:
                last_enemy=i
                break
        # understand if we are racing or fighting
        if last_my>last_enemy:
            rm=True
        if (self.bar_pieces.values()==[[], []])&(rm):
            Race=True
        if Race:
            return 1
        else:
    ...
    def extract_features(self, player):
        ...
        features.append(self.blots(player))
        features.append(self.blocks(player))
        features.append(self.empty_home_slots(player))
    ...
    def next_step(self, player, player_num, draw=False):
        ...
        if draw:
            if player_num==0:
                self.reverse()
                print(self.extract_features(self.opponent(player.player)))
            self.draw()
            if player_num==0:
                self.reverse()
