...
class Model(object):
    def __init__(self, sess, model_path, summary_path, checkpoint_path, restore=False):
        ...
        # lambda decay
        lamda = tf.maximum(0.5, tf.train.exponential_decay(0.5, self.global_step, \
            20000, 0.7, staircase=False), name='lambda')
        
        
        #self.xy=lamda
        # learning rate decay
        alpha = tf.maximum(0.01, tf.train.exponential_decay(0.1, self.global_step, \
            20000, 0.9, staircase=True), name='alpha')

        tf.summary.scalar('lambda', lamda)
        tf.summary.scalar('alpha', alpha)

        # describe network size
        layer_size_input = 297
        layer_size_hidden1 = 85
