...
class Game:
    ...
    def Race(self,player):
        ...
        for i in range(len(self.grid)):
            if self.opponent(player) in self.grid[i]:
                last_enemy=i
                break
        for j in range(last_enemy+1,len(self.grid)):
            if player in self.grid[j]:
                rc=0
                break 
        self.reverse()
    ...
    def extract_features(self, player):
        ...
        for p in [player,self.opponent(player)]:
    ...
    def next_step(self, player, player_num, draw=False):
        ...
        if draw:
            print(self.extract_features(player.player))
            if player_num==0:
                self.reverse()
            self.draw()
            if player_num==0:
    ...
    def get_actions(self, roll, player, nodups=False):
        ...
        for i in mv:
            ...
            for j in range(24):
                if player in self.grid[j]:
                    ...
                    if k<24:
                        ...
                        ff=True
                        for l in range(k,24):
                            if self.opponent(player) in self.grid[l]:
                                ff=False
                                break
                        if ff==True:
            ...
            #print(str(i)+" "+str(max_count_block)) 
            if max_count_block>5:
                for_del.append(i)
        for i in for_del:
            mv.remove(i)
        return mv
