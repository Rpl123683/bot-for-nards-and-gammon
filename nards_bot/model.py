...
#from backgammon.agents.td_gammon_agent_custom import TDAgent1
from backgammon.agents.today import Today_bot
...
def dense_layer(x, shape, activation, name):
    with tf.variable_scope(name):
        ...
        return activation(tf.matmul(x, W) + b, name='activation'), W, b
...
class Model(object):
    def __init__(self, sess, model_path, summary_path, checkpoint_path, restore=False):
        ...
        #self.possible_moves_history=None
        #self.choosen_move=None
        #self.gone_states=None
        # setup our session
        with open("max_wr.txt","r") as file:  
            self.max_wr = file.read()
        self.max_wr=float(self.max_wr)
        ...
        lamda = tf.maximum(0.7, tf.train.exponential_decay(0.9, self.global_step, \
            20000, 0.96, staircase=True), name='lambda')

        # learning rate decay
        alpha = tf.maximum(0.01, tf.train.exponential_decay(0.1, self.global_step, \
            30000, 0.9, staircase=True), name='alpha')
        ...
        layer_size_hidden1 = 80
        layer_size_hidden2 = 60
        layer_size_hidden3 = 40
        layer_size_hidden4 = 20
        layer_size_output = 1
        ...
        prev_y1, self.l1_W, self.l1_b = dense_layer(self.x, [layer_size_input, layer_size_hidden1], tf.sigmoid, name='layer1')
        prev_y2, self.l2_W, self.l2_b = dense_layer(prev_y1, [layer_size_hidden1, layer_size_hidden2], tf.sigmoid, name='layer2')
        prev_y3, self.l3_W, self.l3_b = dense_layer(prev_y2, [layer_size_hidden2, layer_size_hidden3], tf.sigmoid, name='layer3')
        prev_y4, self.l4_W, self.l4_b = dense_layer(prev_y3, [layer_size_hidden3, layer_size_hidden4], tf.sigmoid, name='layer4')
        self.V, self.l5_W, self.l5_b = dense_layer(prev_y4, [layer_size_hidden4, layer_size_output], tf.sigmoid, name='layer5')
    ...
    def play(self):
        game = Game.new()
        game.play([TDAgent(Game.TOKENS[0], self), TDAgent(Game.TOKENS[1], self)], draw=True)
        #self.turn_player=game.turn_player
        #self.roll_history=game.roll_history
        #self.possible_moves_history=game.possible_steps_history
        #self.choosen_move=game.choosen_move
        #self.gone_states=game.gone_states
    def test(self, episodes=100, draw=False, mode=0):
        ...
        if mode==3:
            players = [Today_bot(Game.TOKENS[0]), RandomAgent(Game.TOKENS[1])]
        if mode==4:
    ...
    def train(self):
        ...
        for episode in t:
            ...
            if episode != 0 and episode % validation_interval == 0:
                ...
                np.random.seed()
                #self.test(episodes=100,mode=1)
            t.refresh()
            game = Game.new()
            player_num = random.randint(0, 1)
            learn=int(player_num)
            #print(players[player_num].player)
            #print(game.players[0])
            if players[player_num].player==game.players[0]:
                #print(1)
                game.reverse()
            if episode%2==0:   
                ...
                while not game.is_over():
                    ...
                    #else:
                        #game.reverse()
                        #x_next = game.extract_features(players[learn].player)
                        #V_next = self.get_output(x_next)
                        #game.reverse()
                ...
                game.reverse()
                x = game.extract_features(players[(learn+1)%2].player)
                game.reverse()
                game_step = 0
                while not game.is_over():
                    game.next_step(players[player_num], player_num)
                    ...
                    if player_num!=learn:
                        x_next = game.extract_features(players[(learn+1)%2].player)
                        V_next = self.get_output(x_next)
